...
public class NetworkInputBuilder {
  ...
  private List<NetworkNodeInput> nodeInputs;
  private BayesianNetwork bayesianNetwork;

  public BayesianNetwork buildNetwork(String networkName, List<NetworkNodeInput> nodeInputs) {
  ...
  private void addAllNodes() {
    nodeInputs.forEach(
        nodeInput -> bayesianNetwork.addNewNode(nodeInput.getId(), nodeInput.getStateIds()));
  }

  private void parentNodes() {
    nodeInputs.stream()
        .filter(ni -> ni.getParentIds() != null)
        .forEach(ni -> bayesianNetwork.addParents(ni.getId(), ni.getParentIds()));
  }
  ...
  private ConstraintBuilderIterator buildConstraintBuilderIterator(NetworkNodeInput nodeInput) {
    Serializable[] cptStrideOrderIds = nodeInput.getCptStrideOrderIds();
    ...
    Node event = bayesianNetwork.getNode(nodeInput.getId());
    conditions.remove(event);
    return new ConstraintBuilderIterator(Set.of(event), conditions, vector);
  }

  private List<Node> convertToNodes(Serializable[] cptStrideOrderIds) {
    return Arrays.stream(cptStrideOrderIds).map(bayesianNetwork::getNode).toList();
  }
