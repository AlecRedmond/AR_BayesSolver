...
public class JointConstraintSolverHandler extends BaseConstraintSolverHandler
  ...
  protected void iterateOverConditions(
    ...
    iterator.iterateKeyCombos(
        ...
        conditionPosition,
        iterateConditions,
        (conditionState, index) -> {
          boolean isEvent = checkIsEvidence(conditionState, eventPosition, iterateEvents);
          ObjIntConsumer<int[]> consumer = isEvent ? ifIsEvent : ifNotEvent;
          iterator.iterateKeyCombos(vector, conditionState, iterateEvents, consumer);
        });
  }
}
